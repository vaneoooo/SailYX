...
def wzspider(cp):
    ...
    url = 'http://www.sclzjj.gov.cn/inquiry/Transgress.aspx?number=%E5%B7%9D' + cp[3:] + '&type=%E5%B0%8F%E5%9E%8B%E6%B1%BD%E8%BD%A6'
    try:
...
def weatherspider():
    url = 'http://wap.weather.com.cn/wap/weather/101271005.shtml'
    page = urllib2.urlopen(url)
    soup = bs(page)
    title = soup.strong.text
    post_date = soup.h3.text
    ...
    for d in soup.find_all('dd'):
        day.append(d.text)
    for w in soup.find_all('dt'):
        weather.append(w.text)

    result = zip(day,weather)
    return result
